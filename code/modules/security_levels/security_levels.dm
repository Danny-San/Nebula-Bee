	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != GLOB.security_level)
		switch(level)
			...
			if(SEC_LEVEL_BLUE)
				...
				sound_to_playing_players('sound/misc/voybluealert.ogg', volume = 50) // ogg do citadel pq eu mamo bola do citadel
				for(var/obj/machinery/firealarm/FA in GLOB.machines)
					if(is_station_level(FA.z))
						FA.update_icon()
			if(SEC_LEVEL_RED)
				...
				sound_to_playing_players('sound/misc/voyalert.ogg', volume = 50) // ogg do citadel pq eu mamo bola do citadel
